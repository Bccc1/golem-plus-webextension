...
if(correctViewmode){
  ...
  if(httpRequestRequired){
    console.log("httpRequestRequired")
...
function insertActionBarStyles(){
  Utils.insertCss('.post-hover-class { display: none; }');
  Utils.insertCss('.post-hover-class a { color: rgb(76, 76, 76); }');
  Utils.insertCss('li:hover + .post-hover-class { display: block; padding: 10px 20px; }');
  Utils.insertCss('.post-hover-class:hover { display: block; padding: 10px 20px; }');
  Utils.insertCss('.link-class:hover { text-decoration: none; }');
}
...
function requestDepthsFromThreadViewListener () {
  ...
  console.log("this.responseXML.getElementsByClassName(\"table-listthread\")")
  console.log(this.responseXML.getElementsByClassName("table-listthread"))
